...
# Put nick/code in the QUERY string (macOS `open` preserves ?query but can drop #fragment).
# Unique k= each knock forces a fresh page load instead of focusing an open tab.
url="${URL_BASE}?nick=${nick}&code=${code}&k=$(date +%s)"
